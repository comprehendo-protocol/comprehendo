...
known_issues:
  ...
  - "[gap] `packages/core/src/router-discovery.ts` still reads Router & Precedence [22]'s three PROVISIONAL artifact names (`comprehendo.fingerprints.json`, `comprehendo.twins.json`, `comprehendo.packed.json`) and does not know Corpus Format [28]'s `comprehendo.corpus.json`. This feature is the first real PRODUCER of the new format, so the mismatch is now live and measured, not theoretical: installing this feature's own assembled payload into a real consumer tree and running core's real `discoverInstalledCorpora` finds 0 corpora and reports the defect `declares comprehendoCorpus but carries none of the three corpus artifacts`. Nothing is silent (core reports it), and nothing regresses today (no corpus is published yet). The migration is small and already named by 28: `CORPUS_ARTIFACTS` becomes the one artifact, `loadCorpusFrom` reads it once, and the fingerprints/twins/docs halves come out of it. That file is outside this feature's `source_files` and belongs to 22. See JUDGMENT call 9."
...
- [gap] Core's `router-discovery.ts` still reads 22's three provisional
  artifact names, so the payload this feature assembles is not yet readable
  by 22's discovery adapter. Measured live, named precisely in the
  frontmatter entry, and outside this feature's `source_files`.
