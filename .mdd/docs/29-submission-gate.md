...
known_issues:
  ...
  - "[gap] The danger lint's destructive vocabulary and the injection lint's phrase table are FLOORS, not proofs; neither can be complete. The injection table deliberately rejects second-person prose (\"you must\"), because CC11 [25] says corpus text is about the tool and never addressed to the agent. ffmpeg Corpus [32] (Wave 6) was the first feature to feel that cost, and passed the real gate clean: its topics, twins and fixes carry no \"you must\"/\"you should\"/\"always run\" phrasing anywhere, and the injection lint reports zero violations against it, confirmed live. The floor held for a real, non-trivial corpus; it is still not a proof for corpora not yet written."
