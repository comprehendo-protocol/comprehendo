...
known_issues:
  ...
  - "[resolved by 30-owner-endorsement] Trust DATA now exists: computeEndorsement fills the InstalledCorpus.trust slot this build wired, and require: endorsed is proven end to end through core's real router over a real installed package (endorsed answers sidecar, the same corpus at community answers none/UNSTRUCTURED). JUDGMENT call 3."
  ...
  - "[decision, settled by 30-owner-endorsement] A local corpus carries no trust level, so it reads as community: require: {internal-pkg: endorsed} refuses a corpus the consuming org wrote itself. Owner Endorsement [30] answers this rather than leaving it open: endorsement is read off a package's LIVE PUBLISHED manifest (comprehendo.corpus / comprehendo.owners), and a local corpus has no published manifest to read at all, so it structurally cannot be endorsed by this tier. Not a stub awaiting data; a consuming org that wants require: endorsed on its own private corpus has no channel to satisfy it through this mechanism, by design of what endorsement means (a claim only a real publish credential can make)."
